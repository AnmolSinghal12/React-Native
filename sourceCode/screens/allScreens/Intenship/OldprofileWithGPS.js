        ...
        async (position) => {
          ...
          try {
            const response = await axios.get(
              `https://apis.mapmyindia.com/advancedmaps/v1/9cded52dd0db1bc6e55d14c6f469f2b4/rev_geocode?lat=${latitude}&lng=${longitude}`
            );
            ...
            if (response.data && response.data.results && response.data.results.length > 0) {
              ...
              Show_Toast('error', 'Could not fetch address');
            }
          } catch (error) {
            console.error('Error fetching current city:', error);
            Show_Toast('error', 'Error fetching address');
          }
        },
        (error) => {
          console.error('Geolocation error:', error);
          Show_Toast('error', 'Error fetching location');
        },
      ...
      <SafeAreaView style={{flex: 1, backgroundColor: Colors.whitetxt}}>
        ...
        <ScrollView keyboardShouldPersistTaps="handled">
          ...
          <TouchableOpacity
              ...
              onPress={fetchCurrentCity}
            >
              <Text style={styles.addressText}>
                {address ? address : `${t('Current Address')}`}
              </Text><Image source={require('../../../assets/Images/Address.png')} />
            </TouchableOpacity>
